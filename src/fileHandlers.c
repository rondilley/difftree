                      ...
                      const char *outFile)
{
  ...
  fprintf(out, "BASE=%s\n", base);
  fprintf(out, "MODE=%s\n",
          (config->hash) ? "HASH" : (config->quick) ? "QUICK"
                                : (config->count)   ? "COUNT"
                                                    : "NORMAL");
  ...
  else
  {
    for (i = 0; i < ret; i = i + 2)
    {
      ...
      else if (strcmp(inBuf, "BASE") EQ 0)
      {
        ...
        {
          fprintf(stderr, "ERR - Base directory is corrupted\n");
          deInitParser();
          fclose(inFile);
          return (FAILED);
        }
      ...
      {
        fprintf(stderr, "ERR - Base is corrupted\n");
        deInitParser();
        fclose(inFile);
        return (FAILED);
      }
...
int loadV1File_old(FILE *inFile)
{
  char inBuf[8192];
  char keyString[MAXPATHLEN + 1];
  char startDate[128];
  ...
  /* XXX need to switch to simple key=value| parser */
  if ((ret = parseLine(inBuf)) != 2)
  {
    printf("ERR - Malformed record line - [%d] fields\n", ret);
  }
...
int loadExclusions(char *fName)
{
  ...
  while (fgets(inBuf, MAXPATHLEN, inFile) != NULL)
  {
    ...
    if ((inBuf[0] != '#') && (len > 0))
    {
      ...
      XSTRNCPY(config->exclusions[i], inBuf, MAXPATHLEN);
      config->exclusions[++i] = NULL;
    }
  }
